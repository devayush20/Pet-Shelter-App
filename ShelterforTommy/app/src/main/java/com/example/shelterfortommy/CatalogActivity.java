...
public class CatalogActivity extends AppCompatActivity {
    ...
    private void displayDatabaseInfo() {
        ...
        //Perform the query on the database to obtain the cursor for the table
        Cursor cursor=getContentResolver().query(PetEntry.Content_URI,projection
        ,null,null,null);
        //Get the list view reference
        ListView listview=(ListView) findViewById(R.id.list);

        //Find the view which will be set when no content to display
        View emptyView=findViewById(R.id.empty_view);
        listview.setEmptyView(emptyView);
        //Setup the cursor adapter for the list
        PetCursorAdapter adapter=new PetCursorAdapter(this,cursor,0);//pass 0
